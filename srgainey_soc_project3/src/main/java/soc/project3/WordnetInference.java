...
public class WordnetInference {
	...
	public WordnetInference(String url1, String url2, String url3, String url4) {
		this.sensUrls=url1;
		this.simiUrls=url2;
		this.meroUrls=url3;
		this.antyUrls=url4;
	...
	public static void checkWordUri(String word1,String word2) {
		...
		try {
			...
			FileManager.get().readModel(sch, WNBASIC_OWL);
			Reasoner reasoner = ReasonerRegistry.getOWLReasoner();
			reasoner = reasoner.bindSchema(sch);
			
			Model model = ModelFactory.createDefaultModel();
	     	FileManager.get().readModel(model, WORDNET_CORE);
			
			InfModel read = ModelFactory.createInfModel(reasoner, model);
		...
		*/
		if(!wd1Uri.isEmpty()) {
			if(checkPartMeronyms(wd1Uri,wd2Uri,word1,word2)) {
				return;
			}	
		}
		/*
	...
	*/
	...
	public static Boolean checkPartMeronyms(String Uri1,String Uri2,String word1,String word2) {
		Model model = ModelFactory.createDefaultModel();     	
		FileManager.get().readModel(model, WORDNET_MERONYM_PART);

		Model sch = ModelFactory.createDefaultModel();
     	FileManager.get().readModel(sch, WNBASIC_OWL);
		Reasoner rea = ReasonerRegistry.getOWLReasoner();
		rea = rea.bindSchema(sch);		
		InfModel read = ModelFactory.createInfModel(rea,model);
		Resource res = read.getResource(Uri1);
		StmtIterator stmt = res.listProperties();
		while(stmt.hasNext()) {
			Statement s = stmt.next();
	...
	public static Boolean checkPartHolonyms(String Uri1,String Uri2) {
		Model model = ModelFactory.createDefaultModel();     	
		FileManager.get().readModel(model, WORDNET_MERONYM_PART);
